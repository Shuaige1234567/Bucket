...
actor bucketTest{
    ...
    public shared(msg) func test_init() : async (){
        switch(await bucket.put(#init(init_chunk))){
    ...
    // 99 times
    public shared(msg) func test_append() : async (){
        let test_append = {
            chunk = chunk;
            key = key;
            order = order;
        ...
        order := order + 1;
        switch(await bucket.put(#append(test_append))){
    ...
    public shared(msg) func test_get() : async (){
        var i = 0;
        label l loop{
            if(i == times){
                Debug.print("get end");
            ...
            switch(await bucket.get({key = key; flag = i})){
