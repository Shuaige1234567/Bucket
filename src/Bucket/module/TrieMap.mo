...
module {
  ...
  public class TrieMap<K, V> (isEq : (K, K) -> Bool, hashOf :  K -> Hash.Hash) {
    ...
    private type Trie<K, V> = T.Trie<K, V>;

    public func getMap() : Trie<K, V> {
        map
    };

    public func getArray() : [(K,V)]{
        T.toArray<K,V,(K,V)>(map,func(k:K,v:V):(K,V){(k,v)})
    };

    public func fromTrie(trie : Trie<K, V>){
        map := trie;
    };
